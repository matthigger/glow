...
to a shared S3 prefix; when the array drains, the records are pulled down into
the local records tree the benchmark read path reads (glow._extra.benchmark
make_csv / plot). The work split needs no per-cell shipping -- the array index
addresses a cell, since the cell enumeration is a pure function of the
catalogue.
...
Modules:
    ...
    driver   -- drive_aws: submit, escalate OOM tiers, pull the records home.
