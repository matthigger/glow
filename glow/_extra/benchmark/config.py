...
# added here as a further arm -- one entry, plus a plot.COLOR_ANALYSIS colour
# -- but it draws n_perm_inner + 1 times per outer perm, orders of magnitude
# above these arms, so the paper's grid does not carry it.

# The GLOW arm the figures report (benchmark.plot drops Focus and calls this
# one plainly GLOW), and the only arm the runtime caches time: the two run the
# same permutation walk over the same shapes and differ only in what Ward is
# handed, so timing both would buy a second copy of one curve. Detection is
# another matter, so every run_ana cache runs both.
