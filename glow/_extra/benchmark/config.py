...
The two upstream grids come from builder functions whose keyword arguments are
the swept axes: ``get_kwargs_data_list`` returns the list of data_factory kwargs
over (source, b, num_img, seed); ``get_kwargs_effect_list`` returns the list of
effect_factory kwargs over (effect_llr, n_vox) -- or ``[None]`` for the null
path. Each cache calls them with the axes it sweeps, leaving the rest at their
defaults. The leaf is run_ana over the shared analysis recipes (RUN_ANA_LIST).
...
trial fn; here a cell carries the Extenter (effect support, analysis crop) and
the realized HCP feature subset directly, so the catalogue is what runs -- no
factory indirection.
...
realization. The driver decouples the data and effect loops (one effect grid is
shared across all data cells), so here a data cell's ``seed`` drives its whole
realization (the WGN draw / HCP feature subset, the x design, and the analysis
crop location), and the effect support is placed with ``seed_from_exp`` --
effect_factory derives its placement seed from a hash of the experiment, so each
realization gets its own (reproducible) effect location even though the effect
grid is shared and carries no per-data seed (see effect_factory).
...
clean power curve, and across the extent (n_vox) sweep the whole-region LLR grows
with the region. (The factory has no whole-region-LLR knob -- only the per-voxel
effect_llr -- so an extent sweep at fixed total would have to back it out here;
we don't.)

WGN and HCP share each cache (both sources in one data grid); they face apart on
the recorded source column downstream. HCP has no num_img axis (its N is the
cohort), so the num_img sweep is WGN-only.
...
segmentation oracle, a shared voxel-stat walk, two pruning rules on one fit, two
planted effects, per-perm staircases); each becomes its own ``fnc`` +
...
ana_kwargs_dict = {
    ...
    'GLOW-GLM':   AnalysisGLOW(n_perm_inner=N_PERM_INNER,
                               ...
                               **kwargs),
    'VBA':        AnalysisVBA(z_flag=True, tfce_flag=False, get_stat=get_hotel_tr,
                              **kwargs),
...
def get_kwargs_effect_list(*, llr_list=(MODERATE_EFFECT_LLR,),
    ...
    support from -- the ExtenterMinVar class, n_vox, and ``seed_from_exp=True``
    so the placement is derived from the experiment (see the module docstring).
...
# Building the grids runs no experiments and reads no data -- the cells are just
# kwargs (the Extenters are built later, in effect_factory / data_factory).
CONFIG = {
