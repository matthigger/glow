"""Archive the local joblib cache + records and clear the shared AWS S3 cache.
...
prefix (glow._extra.aws.infra) so AWS workers stop warm-resuming from a stale
cache too.
...
def clear_aws_cache(config_path: Optional[str] = None) -> None:
    """Delete the shared S3 cache/ prefix, best effort (warns on failure).

    Routes through the infra clear_storage path (cache prefix only) so AWS
    workers stop warm-resuming from a stale synced cache. A missing AWS config
    or any boto3 error is reported and swallowed: the local archive is the
    primary action, and AWS may not be configured on every machine.
    ...
    try:
        ...
        print(f'[mv_cache] no AWS config at {path}; skipping S3 cache clear')
        return

    args = SimpleNamespace(runs=False, records=False, cache=True, yes=True)
    ...
    try:
        ...
        print(f'[mv_cache] S3 cache clear failed ({exc!r}); skipping')
...
def main(argv=None) -> None:
    ...
    parser = argparse.ArgumentParser(
        ...
        description='Archive the local joblib cache and records to a dated '
                    'old/ folder and clear the shared AWS S3 cache.')
