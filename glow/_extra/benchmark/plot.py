...
plot_prune draws one grid per mode. segment_perc puts the same three Ward modes
against frac_segment, the share of the images the tree is built on;
segment_perc_llr crosses the two, so plot_segment_llr holds the Ward mode fixed
and draws the llr sweep once per fold share, and plot_segment_compare pages the
same frame by fold share, with the Ward modes and the cohorts side by side.
...
def plot_segment_llr(label: str, df, out) -> None:
    ...
    The fold sweep read along the effect axis rather than across it: where
    segment_perc fixes the effect and varies the Ward mode, this fixes the mode
    (one source x metric grid each) and draws the whole llr sweep once per fold
    share, so reading down a panel's curves gives what a smaller segmentation
    fold costs at that effect strength.
...
def main(argv=None) -> None:
    ...
    way past -- and dispatches by family:
      ...
      - segment / prune: tidy_segment / tidy_prune as a source x metric grid,
        prune one grid per Ward mode. Which of the three segment cuts a cache
        gets comes off its own grids (_segment_perc).
