...
``ana.fit(exp)``, and returns the detection score of the discovered
effects against the target -- no data building, no effect planting (the
config layer wraps this with those). It is memoised + recorded exactly
like the data.py builders: it shares their MEMORY / RECORDER, so a run is
cached on disk and joins the same provenance DAG -- a run_ana record's
``exp`` input links to the build (data_factory, or effect_factory's
plant) that produced it, so ``RECORDER.flatten_to_df`` chains data ->
(plant ->) score into one row.
...
already baked into ``exp``, so folding the score in adds no redundant
refits. The one method-uniform surface every Analysis exposes
(effect_list + pval) is read inside score_effects; the score dict it
returns is what every method is compared on (see .score).
...
re-fitting -- the bulk of the "re-score" flexibility a separate step
would have bought, at none of the linking cost.
"""
...
def run_ana(exp: Experiment, ana: Analysis, mask_target_list, label=None):
    ...
    returns self), then scores the discovered effects against the planted
    supports with ``score_effects`` -- the uniform detection score a
    ...
    records on its own. ``mask_target_list`` is a deterministic function
    of ``exp`` (the effect was planted into it), so it adds no independent
    cache key axis -- it is there because score_effects needs the realized
    supports, which ``exp`` does not itself carry.
    ...
    cache key stable when the same recipe object is reused, and the result
    deterministic in (exp, ana, mask_target_list). The fitted copy (and
