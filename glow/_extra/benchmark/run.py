...
data -> (plant ->) score into one row. The method name (GLOW-Focus,
...
def glow_fit_for_prune(exp, *, parent_uid: str, n_perm_fwer: int,
    ...
    The prune cache's shared intermediate: one AnalysisGLOWSplit fit reduced
    to the light triple every rule needs -- the Ward tree, the raw per-region
    LLR (the rank key AnalysisGLOWBase._discover also prunes by, the z-score
    fragmenting under pruning), and the FWER-significant region set. All the
    rules prune this same set, so the comparison isolates the rule from the
    permutation test. A plain disk-memoised helper, not a DAG node.
