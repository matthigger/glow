...
The draws come from inner_perm_gpu.gpu_perm_full, which takes the tree and
the experiment the split architecture already fixes: one tree from the
segmentation fold, every statistic on the test fold, base_seed = 0 so row 0
is the observed draw (permute._perm_indices reserves seed 0 for it). There
is no per-outer-perm tree to pool and no nested inner loop to funnel, so
...
def gpu_draws(config: GpuConfig, *, exp, base_seed: int, n_perm: int,
    ...
    The device counterpart of inner_perm.cpu_reliable_full, and a drop-in
    for it at AnalysisGLOW.fit's one call site: same draws, same NaN
    convention, same seed-to-draw mapping. Sizes perm_chunk first (see
    resolve_perm_chunk), then defers to inner_perm_gpu.gpu_perm_full.
    ...
    return inner_perm_gpu.gpu_perm_full(
