...
`DataSourceS3` wraps a benchmark `DataSource` whose `.exp` is too
expensive to rebuild on the worker (e.g. HCP).  The driver builds the
exp locally, uploads it once per unique source, and the worker fetches
from S3 on demand.  Deterministic sources (`DataSourceWGN`) are left
in the trial dict and rebuilt on the worker from their seed.
