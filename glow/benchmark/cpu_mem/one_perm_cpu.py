#!/usr/bin/env python3
"""Profile compute time for local worker-style permutation runs."""
...
def measure_cpu_time(target_vox):
    temp_folder = Path(tempfile.mkdtemp(prefix='glow_cpu_profile_'))
    try:
...
if __name__ == '__main__':
    ...
    csv_path = results_dir / 'one_perm_cpu.csv'

    print('=' * 60)
    print('Worker Permutation CPU Profiling')
    print('=' * 60)
    ...
    for target_vox in voxel_targets:
        sample = measure_cpu_time(int(target_vox))
