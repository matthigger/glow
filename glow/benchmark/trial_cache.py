...
A ``TrialCache`` owns a results csv and answers three questions:
  ...
  1. What kwarg dicts should run (cartesian product over
     ``iter_kwargs``, merged with constant ``kwargs``)?
...
class TrialCache:
    ...
    Attributes:
        folder: Path on disk where results live.  Created if missing.
        df: DataFrame of all saved results so far.  Loaded from
            ``results.csv`` at construction; ``save_result`` keeps it
            in sync as trials complete.
        iter_kwargs: Optional dict; values are iterables.  Cartesian
            product yields one kwarg dict per trial.
        kwargs: Optional dict of constant kwargs merged into every
            yielded dict.

    The trial function passed to a Driver must accept
    ``f(**trial) -> dict | pd.DataFrame``; persistence is the cache's
    ...
    def is_cached(self, trial) -> bool:
        """True if the result for this trial is already in ``self.df``."""
        return stable_hash(trial) in self._cached_hashes()

    def save_result(self, result, trial):
        ...
        ``trial_hash`` lives on ``self.df``'s index (so multi-row
        results share an index value).

        Not safe for concurrent writers; the eventual Driver layer is
        responsible for serializing CSV writes (or batching).
        """
