...
pull is the download counterpart of the workers' record upload
(sync.records_pair): it pulls records/ back to its local dir, so a cancelled or
interrupted run's partial results are recoverable. The test drives the real
s3.download_prefix against FakeS3, with records_pair redirected to a tmp dir so
no real user-data dir is touched.
...
def test_pull_downloads_records(tmp_path, monkeypatch):
