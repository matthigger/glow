"""Tests for glow._extra.benchmark.run: run_ana fit + common-output read-out.
...
covered by test_data.py / test_recorder.py and not re-tested here. These cover
what is novel to run_ana: its (effect_list, pval) return contract across
heterogeneous methods, that it fits a private copy (the caller's recipe is left
un-fitted, which is what keeps the cache key stable), and that it joins the
shared provenance DAG (a run_ana leaf carries the build that produced its exp).
"""
import random
...
# ---------------------------------------------------------------------------
# return contract: (effect_list, pval) in that order, uniform across methods
# ---------------------------------------------------------------------------

class TestContract:
    def test_vba_returns_effect_list_then_voxel_pval(self):
        exp = _exp()
        effect_list, pval = run_ana(exp, AnalysisVBA(n_perm_fwer=15))
        assert isinstance(effect_list, list)
        # VBA p-values are per voxel
        assert pval.shape == (exp.y.shape[2],)

    def test_glow_returns_effect_list_then_region_pval(self):
        exp = _exp(shape=(4, 4, 4), num_img=16)
        effect_list, pval = run_ana(
            exp, AnalysisGLOW(n_perm_fwer=8, n_perm_inner=8))
        assert isinstance(effect_list, list)
        # GLOW p-values are per region: num_vox leaves + (num_vox - 1) internal
        num_vox = exp.y.shape[2]
        assert pval.shape == (2 * num_vox - 1,)
...
# ---------------------------------------------------------------------------
# cache: keyed on the (exp, ana) pair (the novel bit -- ana is a recipe arg)
# ---------------------------------------------------------------------------
...
class TestCacheKeyedOnRecipe:
    ...
    def test_cached_result_matches_compute(self):
        exp = _exp()
        ana = AnalysisVBA(n_perm_fwer=15)
        _, pv0 = run_ana(exp, ana)   # computed
        _, pv1 = run_ana(exp, ana)   # served from cache
        assert np.array_equal(pv0, pv1)
