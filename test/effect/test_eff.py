...
class TestEffect:
    ...
    def test_from_exp_mask(self):
        ...
        eff_direct = glow.effect.Effect.from_x_y_contrast(
    ...
    @pytest.mark.parametrize('eff1, eff2', [
        ...
        (glow.effect.Effect(mask=np.ones((10, 10), dtype=bool),
                            y_mean=np.zeros((2, 10))),
         glow.effect.Effect(mask=np.ones((8, 8), dtype=bool),
                            y_mean=np.zeros((2, 10)))),
        # same shape, different mask values
        (glow.effect.Effect(mask=np.ones((10, 10), dtype=bool),
                            y_mean=np.zeros((2, 10))),
         glow.effect.Effect(mask=np.zeros((10, 10), dtype=bool),
                            y_mean=np.zeros((2, 10)))),
