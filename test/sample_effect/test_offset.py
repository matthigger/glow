...
def test_compute_offset_qr():
    for seed in range(1):
        # generate dummy data
        x, y, contrast = generate_dummy_data(seed=seed)

        for f_stat_exp in (0, 1, 10, 100, 1e6):
            # validate that 1.f stat is achieved
            offset = compute_offset_qr(x=x, y=y, contrast=contrast,
                                       f_stat=f_stat_exp)
...
def test_compare_compute_offsets():
    for seed in range(10):
        ...
        for f_stat_exp in (0, 1, 10, 100, 1e6):
            offset = compute_offset(x=x, y=y, contrast=contrast,
                                    f_stat=f_stat_exp)
            offset_qr = compute_offset_qr(x=x, y=y, contrast=contrast,
                                          f_stat=f_stat_exp)

            np.testing.assert_allclose(offset, offset_qr, rtol=1e-1, atol=1e-1)
