...
class TestPrepDf:
    def test_shape(self, ana, exp, df_no_target):
    ...
    def test_region_idx_contiguous(self, ana, exp, df_no_target):
        num_vox = exp.y.shape[2]
        num_reg = num_vox + ana.children.shape[0]
    ...
    def test_leaves_have_size_one(self, ana, exp, df_no_target):
        num_vox = exp.y.shape[2]
...
class TestComputeTargetStats:
    ...
    def test_n_voxel_matches_mask(self, exp, mask_target, target_stats):
    ...
    def test_empty_mask_returns_none(self, exp):
        empty_mask = np.zeros_like(exp.mask_idx, dtype=bool)
        assert compute_target_stats(exp, empty_mask) is None


class TestComputeBackgrounds:
    """Tests for compute_backgrounds including per-image selection."""

    def test_mean_returns_dict(self, ana, exp):
        bg = compute_backgrounds(exp)
        assert isinstance(bg, dict)
        assert len(bg) >= 1

    def test_mean_shapes_match_mask(self, exp):
        bg = compute_backgrounds(exp)
        mask_shape = exp.mask_idx.shape
        for name, img in bg.items():
            assert img.shape[:len(mask_shape)] == mask_shape

    def test_single_image_returns_dict(self, ana, exp):
        bg = compute_backgrounds(exp, image_idx=0)
        assert isinstance(bg, dict)
        assert len(bg) >= 1

    def test_single_image_differs_from_mean(self, ana, exp):
        bg_mean = compute_backgrounds(exp)
        bg_0 = compute_backgrounds(exp, image_idx=0)
    ...
    def test_all_image_indices_valid(self, exp):
        num_img = exp.y.shape[1]
        for i in range(num_img):
            bg = compute_backgrounds(exp, image_idx=i)
            assert isinstance(bg, dict)

    def test_y_features_used_as_keys(self, exp):
        names = [f'feat_{i}' for i in range(exp.y.shape[0])]
        bg = compute_backgrounds(exp, y_features=names)
        for n in names:
            assert n in bg
